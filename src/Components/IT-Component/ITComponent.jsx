...
function ITComponent(props) {
  ...
  const parseResponseString = (responseString) => {
    ...
    return questions.map((question) => {
      ...
      const optionsAndCode = questionText
        .split("\n")
  ...
  const handleChange = (index, event) => {
    ...
    newFormData[index][name] =
      name === "concepts"
        ? value.split(",").map((concept) => concept.trim())
        : value;
      ...
      formData.forEach((form, index) => {
        Object.keys(form).forEach((key) => {
          ...
          const errorMsg = validateField(
            key,
            key === "concepts" || key === "toolsTechnologies" ? value : value,
            index
          );
          if (errorMsg) {
            isValid = false;
      ...
      const formattedData = cleanedData.map((item) => {
        ...
        item.options.forEach((option, index) => {
          if (item.codeSnippet == null) {
            formattedItem[`option${index + 1}`] =
              index === 0 ? option : option.substring(2); // Trim first two characters
          } else {
        ...
        if (typeof item.answer === "string") {
          formattedItem.answer = item.answer
            .replace(/^\*\*/, "")
            .replace(/\*\*$/, ""); // Remove '**' at start and end
          if (formattedItem.answer.startsWith(".")) {
            const dotIndex = formattedItem.answer.indexOf(".");
            formattedItem.answer =
              dotIndex !== -1
                ? formattedItem.answer.substring(0, dotIndex)
                : formattedItem.answer;
          } else {
            // const dotIndex = formattedItem.answer.indexOf('.');
            formattedItem.answer = formattedItem.answer
              .substring(formattedItem.answer.indexOf(".") + 1)
              .trim(); // Trim text before first '.'
          }
        } else {
          formattedItem.answer = item.answer;
        }
            ...
            <div key={index} className="form-roww">
              <div className="programmingLanguage">
                <label>
                  ...
                  <span
                    className="info-icon"
                    title="Select the specialization for the questions"
                  >
                    i
                  </span>
                  <div className="error">
              ...
              <div className="programmingLanguage">
                <label>
                  ...
                  <span
                    className="info-icon"
                    title="Select the Programming for the questions"
                  >
                    i
                  </span>
                  <div className="error">
                    {errors[`programmingLanguage${index}`]}
                  </div>
                </label>
                <select
              ...
              <div className="concept-input-container">
                ...
                <TagInput
                  ...
                  onTagsChange={(newTags) =>
                    handleChangeToolsTechnologies(index, newTags)
                  }
                />
                <div className="error">
